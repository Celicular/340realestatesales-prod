...
const NewListings = () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.data.length > 0) {
            ...
            console.log('  - createdAt:', firstProp.createdAt, typeof firstProp.createdAt);
            console.log('  - updatedAt:', firstProp.updatedAt, typeof firstProp.updatedAt);
            console.log('  - listedAt:', firstProp.listedAt, typeof firstProp.listedAt);
            console.log('  - timestamp:', firstProp.timestamp, typeof firstProp.timestamp);
          }
  ...
  if (filteredProperties.length === 0) {
    return (
      <section className="py-12 px-4 text-center">
        ...
        <p className="mt-2 text-gray-600">No new listings available at the moment.</p>
      </section>
    );
  }
  ...
  return (
    <section className="py-12 px-4 text-center">
      ...
      {/* Carousel Arrows - Enhanced Professional Design */}
      {filteredProperties.length > 0 && (
        <div className="flex justify-end mt-6 gap-4 pr-8">
          <button 
            onClick={goToPrevious}
            className={`group relative p-4 text-white transition-all duration-300 rounded-full shadow-xl border-2 ${
              filteredProperties.length <= 2 
                ? 'bg-gray-400/50 cursor-not-allowed opacity-50 border-gray-300' 
                : 'bg-gradient-to-r from-[#d4b196] to-[#c4a185] hover:from-[#c4a185] hover:to-[#d4b196] hover:scale-110 hover:shadow-2xl border-white/20 backdrop-blur-sm'
            }`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 max-w-6xl mx-auto">
        {currentProperties.map((property, index) => (
          <div key={property.id || index} className="bg-white shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 rounded-lg overflow-hidden border border-gray-100">
